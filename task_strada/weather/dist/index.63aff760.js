...
function render(response) {
    const id = response.id, city = response.name, temperature = (response.main.temp - 273.5).toFixed(1), feelsLike = (response.main.feels_like, 5).toFixed(1), weather = (0, _supportFunctionsJs.getWeather)(response), sunrise = (0, _supportFunctionsJs.getTime)(response.sys.sunrise), sunset = (0, _supportFunctionsJs.getTime)(response.sys.sunset);
...
function deleteAndCurrentCity(e) {
    ...
    if (e.target.className === "delete-city") {
        ...
        for (const item of locations)if (item.name === cityName) {
